...
matfn=u'E:/声音小组/LQPHP/audiotrack/matlab_code/handwriting/20181127wxy_812_vivo_A_Z_multicos_pauseRecordDemo/label.mat'
data = sio.loadmat(matfn)   #训练集标签
label = data['label']
label = np.transpose(label)

matfn1=u'E:/声音小组/LQPHP/audiotrack/matlab_code/handwriting/picture_20181126lqp_812_vivo_A_Z_multicos_pauseRecordDemo/label.mat'
data1 = sio.loadmat(matfn1)  #测试集标签
label1 = data1['label']
label1 = np.transpose(label1)

# 批量读取灰度图片-训练集
data_dir='E:/声音小组/LQPHP/audiotrack/matlab_code/handwriting/20181127wxy_812_vivo_A_Z_multicos_pauseRecordDemo/gray_picture'
str1=data_dir + '/*.jpg'
...
for n in range(0,len(coll)):
    # print('n:\n',n)
    img = coll[n].reshape(-1,coll[n].shape[0],coll[n].shape[1],1)
    colll.append(img)
...
# 批量读取灰度图片-测试集
#data_dir='E:/声音小组/LQPHP/audiotrack/matlab_code/handwriting/picture_20181126lqp_812_vivo_A_Z_multicos_pauseRecordDemor/picture_gray'
#str1='E:/声音小组/LQPHP/audiotrack/matlab_code/handwriting/picture_20181126lqp_812_vivo_A_Z_multicos_pauseRecordDemor/picture_gray/*.jpg'
coll_1 = io.ImageCollection('E:/声音小组/LQPHP/audiotrack/matlab_code/handwriting/picture_20181126lqp_812_vivo_A_Z_multicos_pauseRecordDemo/picture1_gray/*.jpg')
print(len(coll_1))
img1=[]
colll_1=[]
for k in range(0,len(coll_1)):
    # print('n:\n',n)
    img1=[]
    img1 = coll_1[k].reshape(-1,coll_1[k].shape[0],coll_1[k].shape[1],1)
    colll_1.append(img1)
collll_1 = np.concatenate(colll_1,axis=0)
print('collll_1.shape:',collll_1.shape)
...
print('tset_X.shape',test_X.shape)
print('train_y.shape:',train_y.shape)
print('tset_y.shape',test_y.shape)
N = 26  # N为类别个数
acc=np.zeros((8,1))
re=[]
ma=[]
...
#
model = Sequential()
model.add(Conv2D(32,(2,2),strides=(2,2),input_shape=(data_X.shape[1], data_X.shape[2],1),padding='valid',activation='tanh',kernel_initializer='glorot_uniform'))
model.add(MaxPooling2D(pool_size=(3,3),strides=(2,2)))
model.add(Conv2D(64,(5,5),strides=(1,1),padding='same',activation='tanh',kernel_initializer='uniform'))
model.add(MaxPooling2D(pool_size=(4,4),strides=(2,2)))
# model.add(Conv2D(64,(2,2),strides=(2,2),padding='same',activation='tanh',kernel_initializer='uniform'))
# model.add(Conv2D(64,(3,3),strides=(2,2),padding='same',activation='tanh',kernel_initializer='uniform'))
...
history=model.fit(data_X,train_Y,epochs=1500,batch_size=10,validation_data=(data_Y,test_Y))
...
data.save('E:/声音小组\LQPHP/audiotrack/matlab_code/mousedetect/data/806HPhxy_9种识别/handwriting_STFTCNN1_trainwxy_testlqp_feature_512_RMSprop.xls')
